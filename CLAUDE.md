...
|---|---|---|
| [Execution plan/sozialphilosophie_syllabus.md](Execution plan/sozialphilosophie_syllabus.md) | The intellectual map. Annotated bibliography, dependency graph, [P]/[ES]/[RS]/[O]/[NE] markers plus "Marxist deepening" labels (formerly [MX], retired 2026-07-14), [Economist's note] callouts, Builds on/Leads to/**Contested by** dependency fields, accelerated path, appendices F/G for Phase IV. Also carries the per-text hour-formula inputs — `Lang`/`pages`/`chapters`/`Klasse` — merged in from the retired `reading_speed_classes.md` (2026-07-31). **Carries no hour estimates or totals**: hours are derived and live in the execution plan. Reference, not schedule. | Rarely; structural only. |
| [Execution plan/programme_parameters.md](Execution plan/programme_parameters.md) | The spec. Programme parameters (timeline, hour budget, tiered active-practice regime, hybrid cut strategy with Context Layer), structural requirements for the execution plan, falling-behind protocols, session-structure rules by text type, community engagement track, dialogue moments, concentrated time blocks. Distinguishes **fixed** from **revisable** parameters (§6 Class A/B/C table). | When parameters change — but check fixed/revisable table first. |
...
| [Execution plan/reading_speed_classes.md](Execution plan/reading_speed_classes.md) | **Retired 2026-07-31 — now a redirect.** Per-text classification moved into the syllabus tables; the framework (5 difficulty classes, pp/h rates, indicative authors, special rules) moved to `reading_practices.md` § Appendix G. The stub retains the reconciliation record. | Don't add to it; classify in the syllabus. |
...
- Don't break the [P]/[ES]/[RS]/[O]/[NE] marker system, the "Marxist deepening" labels (formerly [MX]), or the Builds on / Leads to / Contested by dependency fields when editing the syllabus or plan. Item IDs were globally renumbered 2026-07-14 — the old→new mapping table lives at the end of `execution_plan_changelog.md`; archive/ and print/ retain old IDs. Appendix C IDs C.1–C.5 are **retired and deliberately vacant** (extracted to Module 13, 2026-07-31); don't reuse them.
- **Don't store derived values.** Hours are `f(pages, prose class, tier, α)` — compute them in the execution plan, never write them into the syllabus. An audit on 2026-07-30 found the syllabus's Part totals had drifted about a third away from its own item rows, with four appendices carrying three mutually inconsistent totals each; the 2026-07-31 merge removed the whole class of error by removing the stored values. The same rule applies to anything else that can be recomputed from canonical inputs.
- **Don't put canonical inputs in `execution_plan.md`.** It is a derivative document and may be rebuilt from scratch; anything written only there is lost on rebuild. Prose-class rates live in `reading_practices.md` § Appendix G for exactly this reason.
